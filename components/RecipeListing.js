...
export default function RecipeListing({
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(sortedRecipes.length / ITEMS_PER_PAGE),
  );
  ...
  const currentRecipes = sortedRecipes.slice(start, start + ITEMS_PER_PAGE);
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 lg:px-12 lg:py-20">
        <div className="text-center text-[#253C57]">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-base text-[#4B433C] sm:text-lg">
            Browse plant-powered plates, drinks, and treats that fit your mood.
            Use the filters to find new favorites and keep your menu feeling
            fresh.
          </p>
        </div>
          ...
          {currentRecipes.length === 0 ? (
            <div className="col-span-full rounded-3xl bg-white/80 p-12 text-center text-[#4B433C]">
              <p className="text-base sm:text-lg">
                No recipes in this category yet, but new creations are always on
                the way.
              </p>
            </div>
          ) : (
            currentRecipes.map((recipe) => {
              ...
              const href = recipe.slug ? `/recipes/${recipe.slug}` : pathname;
