...
The SQL Server connection. PowerFab has user accounts in its database. SQL Server checks the username and password and decides whether to let the connection through.
The FabSuite API call. Look at PowerFabApiClient.cs lines 144–148 — every request includes a <Connect> block with a username and password. The DLL forwards that to the FabSuite server, which decides whether to honor the request.
