...
1. **First time:** They download `PowerFab Dashboard Setup 1.x.x.exe` from your customer portal (or wherever you host releases — doesn't really matter where, as long as they can reach it).
2. They double-click. Windows shows them an installer wizard. They click *Next, Next, Install*.
3. They see a Start menu entry and a desktop shortcut. They click the shortcut.
4. The app opens. They see a login screen. They type their PowerFab username and password (and possibly the address of their PowerFab server, if it's not auto-discovered — see "Things still to figure out" below).
5. They use it.
6. Next time, they just click the shortcut.
