...
Stay hosted (the original SaaS plan)
Customer-hosted server, thin client per user
Pure C# desktop app (WPF / WinForms / MAUI / Avalonia)
Pure web app rewrite where the user installs nothing
Rewriting everything in TypeScript / Node.js
Direct-to-database client with no API at all
Tauri instead of Electron (we may revisit)
Bundling the Tekla DLL ourselves
